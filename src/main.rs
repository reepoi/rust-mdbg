...
use glob::glob;
use lzzzz::lz4f::{BufReadDecompressor, Preferences, WriteCompressor}; //, PreferencesBuilder, CLEVEL_HIGH};
...
fn main() {
    ...
    if opt.k.is_none() && opt.l.is_none() && opt.density.is_none() {
        ...
        if opt.k.is_some() {
            ...
            println!("Warning: Using default k value ({}).", k);
        }
        if opt.l.is_some() {
            l = opt.l.unwrap()
        } else {
            println!("Warning: Using default l value ({}).", l);
        }
        if opt.density.is_some() {
            density = opt.density.unwrap()
        } else {
            println!(
                "Warning: Using default density value ({}%).",
                density * 100.0
            );
        }
    }
    if opt.n.is_some() {
        n = opt.n.unwrap()
    } else if error_correct {
        println!("Warning: Using default n value ({}).", n);
    }
    if opt.t.is_some() {
        t = opt.t.unwrap()
    } else if error_correct {
        println!("Warning: Using default t value ({}).", t);
    }
    if opt.minabund.is_some() {
        min_kmer_abundance = opt.minabund.unwrap() as DbgAbundance
    } else {
        println!(
            "Warning: Using default minimum k-mer abundance value ({}).",
            min_kmer_abundance
        );
    }
    if opt.presimp.is_some() {
        presimp = opt.presimp.unwrap();
    } else {
        println!("Warning: Using default pre-simp value (0.01).");
    }
    if opt.threads.is_some() {
        threads = opt.threads.unwrap();
    } else {
        println!("Warning: Using default number of threads (8).");
    }
    if opt.correction_threshold.is_some() {
        correction_threshold = opt.correction_threshold.unwrap()
    } else if error_correct {
        println!(
            "Warning: using default correction threshold value ({}).",
            correction_threshold
        );
    }
    if opt.distance.is_some() {
        distance = opt.distance.unwrap()
    }
    if distance > 2 {
        distance = 2;
    }
    ...
    if opt.distance.is_none() && error_correct {
        println!(
            "Warning: Using default distance metric ({}).",
            distance_type
        );
    }
    if opt.restart_from_postcor {
        restart_from_postcor = true;
    }
    if opt.bf {
        use_bf = true;
    }
    ...
    if use_syncmers {
        if opt.s.is_some() {
            ...
            println!("Warning: Using default s value ({}).", s);
        }
    }
    if opt.read_stats.is_some() {
        read_stats = opt.read_stats.unwrap();
    }
    if opt.no_basespace {
        no_basespace = true;
    }
    ...
    if opt.lmer_counts.is_some() {
        ...
        if opt.lmer_counts_min.is_some() {
            lmer_counts_min = opt.lmer_counts_min.unwrap();
        } else {
            println!(
                "Warning: Using default l-mer minimum count ({}).",
                lmer_counts_min
            );
        }
        if opt.lmer_counts_max.is_some() {
            lmer_counts_max = opt.lmer_counts_max.unwrap();
        } else {
            println!(
                "Warning: Using default l-mer maximum count ({}).",
                lmer_counts_max
            );
        }
    }
    ...
    if opt.prefix.is_some() {
        output_prefix = opt.prefix.unwrap();
    } else {
        println!(
    ...
    if has_lmer_counts {
        ...
        loop {
            ...
            let new_line = |line: &mut String, br: &mut BufReader<File>| {
                line.clear();
                br.read_line(line).ok();
            };
            if let Err(_e) = br.read_line(&mut line) {
                break;
            }
            if line.is_empty() {
                break;
    ...
    // parsing
    let queue_len = if _mean_length >= 1000000 { 32 } else { 200 }; // not too many seqs in cache if we're parsing reference genomes
                                                                    // note: this effectively limits fasta parsing of long
                                                                    // sequences to 32 threads, which should be fine for human

    let mut uhs_bloom: RacyBloom = RacyBloom::new(Bloom::new(
        if use_bf { 500_000_000 } else { 1 },
        1_000_000_000_000_000,
    ));
    let mut lcp_bloom: RacyBloom = RacyBloom::new(Bloom::new(
        if use_bf { 500_000_000 } else { 1 },
        1_000_000_000_000_000,
    ));
    ...
    let bloom: RacyBloom = RacyBloom::new(Bloom::new(
        if use_bf { 500_000_000 } else { 1 },
        1_000_000_000_000_000,
    )); // same bf but making sure we use only 1 hash function for speed
    ...
    for path in glob(&format!("{}*.sequences", output_prefix.to_str().unwrap()))
    ...
    let seq_write = |file: &mut SeqFileType, s| {
        let _res = write!(file, "{}", s);
    };
    ...
    let create_sequences_file = |thread_id: ThreadIdType| -> SeqFileType {
        let seq_path = PathBuf::from(format!(
            "{}.{}.sequences",
            output_prefix.to_str().unwrap(),
            thread_id
        ));
    ...
    let add_kminmer = |node: &Kmer,
                       seq: Option<&str>,
                       seq_reversed: &bool,
                       origin: &str,
                       shift: &(usize, usize),
                       sequences_file: &mut SeqFileType,
                       thread_id: usize,
                       read_seq: Option<&str>,
                       read_offsets: Option<(usize, usize, usize)>| {
        ...
        if use_bf && (!params.reference) && params.min_kmer_abundance > 1 {
            ...
            if contains_key {
                ...
                previous_abundance = 0;
                let seqlen = match seq {
                    Some(read) => read.len() as u32,
                    None => read_offsets.unwrap().2 as u32,
                };
        ...
        if contains_key {
            ...
            dbg_nodes.insert(
                ...
                DbgEntry {
                    index: cur_node_index,
                    abundance: previous_abundance + 1,
                    seqlen,
                    shift: lowprec_shift,
                },
            );
        ...
        if params.reference || previous_abundance >= 1 || min_kmer_abundance == 1 {
            // now record what we will save
            if params.error_correct && thread_id != 0 {
                return;
            } // in error correct mode, only write sequences during second pass
            if previous_abundance == (min_kmer_abundance - 1) {
                ...
                let seq = match seq {
                    Some(read) => read,
                    None => &read_seq.unwrap()[read_offsets.unwrap().0..read_offsets.unwrap().1],
                };
                let seq = if *seq_reversed {
                    utils::revcomp(&seq)
                } else {
                    seq.to_string()
                };
    ...
    if !restart_from_postcor {
        ...
        if error_correct || reference {
            // write this file even for an uncorrected reference, needed by evaluate_ec.py
        ...
        let process_read_aux = |seq_str: &[u8], seq_id: &str| -> Found {
            ...
            let seq_for_ref = if reference {
                seq.replace("\n", "").replace("\r", "") // seq_io might return newlines in fasta seq
            } else {
                String::new()
            };
            let seq = if reference {
                seq_for_ref
            } else {
                seq.to_string()
            };
            let read_obj = Read::extract(
                &seq_id,
                seq,
                &params,
                &minimizer_to_int,
            ...
            */
            ...
            if read_obj.transformed.len() > k {
                ...
                for i in 0..(read_obj.transformed.len() - k + 1) {
                    ...
                    let shift = (
                        position_of_second_minimizer,
                        position_of_second_to_last_minimizer,
                    );
                    let read_offsets = (
                        read_obj.minimizers_pos[i] as usize,
                        (read_obj.minimizers_pos[i + k - 1] as usize + l),
                        (read_obj.minimizers_pos[i + k - 1] + 1 - read_obj.minimizers_pos[i] + 1),
                    );
                    add_kminmer(
                        &node,
                        None,
        ...
        let mut main_thread = |found: &Found| {
            // runs in main thread
            nb_reads += 1;
            ...
            if debug_only_display_read_and_minimizers {
                ...
                println!(
                    "{} {}",
                    &read_obj.id.to_string(),
                    &read_obj.transformed.to_vec().iter().join(" ")
                );
                ...
                if read_obj.transformed.len() >= n {
                    ec_reads::record(
                        ...
                        &read_obj.id.to_string(),
                        &read_obj.seq,
                        &read_obj.transformed.to_vec(),
                        &read_obj.minimizers,
                        &read_obj.minimizers_pos,
                    );
                    if error_correct {
                        for i in 0..read_obj.transformed.len() - n + 1 {
                            let n_mer =
                                utils::normalize_vec(&read_obj.transformed[i..i + n].to_vec());
        ...
        if fasta_reads {
            ...
            let _res = read_process_fasta_records(
                reader,
                threads as u32,
                queue_len,
                process_read_fasta,
                |_record, found| main_thread(found),
            );
        } else {
            let reader = seq_io::fastq::Reader::new(buf);
            let _res = read_process_fastq_records(
                reader,
                threads as u32,
                queue_len,
                process_read_fastq,
        ...
        if error_correct {
            ...
            let ec_entries = Arc::new(Mutex::new(HashMap::<
                usize,
                Vec<(String, String, Vec<u64>, Vec<String>, Vec<usize>)>,
            >::new()));
            let poa_entries = Arc::new(Mutex::new(
                HashMap::<usize, HashMap<String, Vec<String>>>::new(),
            ));
            ...
            if chunks.len() > threads {
                chunk_length = chunks.len() / threads + 1;
            }
            thread::scope(|s| {
            ...
            for thread_num in 0..threads {
                ...
                for (temp, vec) in poa.iter_mut() {
                    ec_reads::record_poa(&mut ec_file_poa, temp, vec.to_vec());
                }
            }
    ...
    if error_correct || restart_from_postcor {
        ...
        for ec_record in chunks.iter() {
            let mut read_obj = Read {
                id: ec_record.seq_id.to_string(),
                minimizers: ec_record.read_minimizers.to_vec(),
                minimizers_pos: ec_record.read_minimizers_pos.to_vec(),
                transformed: ec_record.read_transformed.to_vec(),
                seq: ec_record.seq_str.to_string(),
                corrected: false,
            };
            if read_obj.transformed.len() > k {
    ...
    if min_kmer_abundance > 1 {
        // why we need this conditional:
        ...
        println!(
            "Number of nodes before abundance filter: {}",
            dbg_nodes.len()
        );
        dbg_nodes.retain(|_x, c| c.abundance >= (min_kmer_abundance as DbgAbundance));
        println!(
            "Number of nodes after abundance filter: {}",
            dbg_nodes.len()
        );
    } else {
    ...
    if !read_stats.as_os_str().is_empty() {
        ...
        let read_stats_process_read_fasta =
            |record: seq_io::fasta::RefRecord, _found: &mut Found| {
                let seq_str = record.seq();
                let seq_id = record.id().unwrap().to_string();
                read_stats_process_read_aux(&seq_str, &seq_id);
            };
        let read_stats_process_read_fastq =
            |record: seq_io::fastq::RefRecord, _found: &mut Found| {
                let seq_str = record.seq();
                let seq_id = record.id().unwrap().to_string();
                read_stats_process_read_aux(&seq_str, &seq_id);
            };
        ...
        // parallel fasta parsing, with a main thread that writes to disk and populates hash tables
        let mut read_stats_main_thread = || {
            // runs in main thread
            nb_reads += 1;
            None::<()>
        };

        crate::read_stats::ReadStats::init(
            &read_stats.clone().into_os_string().into_string().unwrap(),
        );
        ...
        if read_stats_filename_str.contains(".fasta.")
            || read_stats_filename_str.contains(".fa.")
            || read_stats_filename_str.ends_with(".fa")
            || read_stats_filename_str.ends_with(".fasta")
        {
            // not so robust but will have to do for now
            read_stats_fasta_reads = true;
            println!("Format: FASTA");
        } else {
            println!("Format: FASTQ");
        }
        if read_stats_fasta_reads {
            ...
            let _res = read_process_fasta_records(
                ...
                threads as u32,
                queue_len,
                read_stats_process_read_fasta,
                |_record, _found| read_stats_main_thread(),
            );
        } else {
            let reader = seq_io::fastq::Reader::new(buf);
            let _res = read_process_fastq_records(
    ...
    for (node, entry) in dbg_nodes_view.iter() {
        ...
        let mut insert_km = |key, val| match km_index.entry(key) {
            Entry::Vacant(ent) => {
                ent.insert(vec![val]);
            }
            Entry::Occupied(mut ent) => {
                ent.get_mut().push(val);
            }
        };
        ...
        for key in [key1, key2].iter() {
            if km_index.contains_key(&key) {
                ...
                for edge in potential_edges.iter() {
                    ...
                    if presimp > 0.0
                        && potential_edges.len() >= 2
                        && (n2_abundance as f32) < presimp * (abundance_ref as f32)
                    {
                        presimp_removed += 1;
    ...
    if presimp > 0.0 {
        ...
        for edge in vec_edges.iter() {
            ...
            if removed_edges.contains(&(*n1_index, *n2_index))
                || removed_edges.contains(&(*n2_index, *n1_index))
            {
    ...
    println!(
        "Maximum RSS: {:?}GB",
        (get_memory_rusage() as f32) / 1024.0 / 1024.0 / 1024.0
    );
}
